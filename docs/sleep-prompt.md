...
You are running sleep mode to optimize long-term memory quality conservatively.
...
1. Create a pre-maintenance export with `mnemosyne export --no-embeddings` and classify proposed changes as one of:
   - exact duplicate;
...
3. If the proposal would delete more than 25 memories or more than 10% of the collection, whichever threshold is reached
   ...
   explicit approval.
4. Apply approved changes. Add and verify every consolidation or replacement with `mnemosyne add [memory content] --tag tag1 --tag tag2` before deleting its source memories with `mnemosyne delete [memory id]`. Move memories between core and regular storage with `mnemosyne update [memory id] --tag core` or `mnemosyne update [memory id] --replace-tags --tag tag1 --tag tag2`; core is for critical, frequently accessed context only.
